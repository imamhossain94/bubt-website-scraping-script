...
def uploadImageFile(fileName, stringFile):
    try:
        ...
        im.save('faculties/eco/%s_%d.png'%(fileName, randId), 'PNG')
        bucket = storage.bucket()
        blob = bucket.blob('faculties/eco/%s_%d.png'%(fileName, randId))
        blob.upload_from_filename('faculties/eco/%s_%d.png'%(fileName, randId))
        blob.make_public()
...
def getECOFaculties(facultyUrl):
    finalData = {'data': list()}
    try:
...
getECOFaculties("/home/faculty_member/economics")

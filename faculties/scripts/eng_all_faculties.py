...
def uploadImageFile(fileName, stringFile):
    try:
        ...
        im.save('faculties/eng/%s_%d.png'%(fileName, randId), 'PNG')
        bucket = storage.bucket()
        blob = bucket.blob('faculties/eng/%s_%d.png'%(fileName, randId))
        blob.upload_from_filename('faculties/eng/%s_%d.png'%(fileName, randId))
        blob.make_public()
...
def getENGFaculties(facultyUrl):
    finalData = {'data': list()}
    try:
...
getENGFaculties("/home/faculty_member/english")

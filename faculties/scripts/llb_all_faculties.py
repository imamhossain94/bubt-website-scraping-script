...
def uploadImageFile(fileName, stringFile):
    try:
        ...
        im.save('faculties/llb/%s_%d.png'%(fileName, randId), 'PNG')
        bucket = storage.bucket()
        blob = bucket.blob('faculties/llb/%s_%d.png'%(fileName, randId))
        blob.upload_from_filename('faculties/llb/%s_%d.png'%(fileName, randId))
        blob.make_public()
...
def getLLBFaculties(facultyUrl):
    finalData = {'data': list()}
    try:
...
getLLBFaculties("/home/faculty_member/law-justice")

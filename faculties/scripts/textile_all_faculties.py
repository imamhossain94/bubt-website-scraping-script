...
def uploadImageFile(fileName, stringFile):
    try:
        ...
        im.save('faculties/textile/%s_%d.png'%(fileName, randId), 'PNG')
        bucket = storage.bucket()
        blob = bucket.blob('faculties/textile/%s_%d.png'%(fileName, randId))
        blob.upload_from_filename('faculties/textile/%s_%d.png'%(fileName, randId))
        blob.make_public()
...
def getTextileFaculties(facultyUrl):
    finalData = {'data': list()}
    try:
        facultyUrl = get(baseURL+facultyUrl).text
        for i in range(22):
            div = BeautifulSoup(str(facultyUrl), 'html.parser').find_all('div', class_="faculty-member faculty-member%d"%(i))[0]
            h3 = str(div.find('h3'))
...
getTextileFaculties("/home/faculty_member/textile")
